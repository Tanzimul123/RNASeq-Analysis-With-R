countdata <- read.csv('Mouse expression dataset.csv')
View(countdata)
...
write.csv(countdata_unique,'modified_mouse_dataset.csv',row.names = FALSE)
#LOADING THE MODIFIED DATA
countdata_unique<-read.csv('modified_mouse_dataset.csv',header = TRUE, row.names = 1)
#ROW.NAMES=1 TELLS R to consider the first column as row names not data
